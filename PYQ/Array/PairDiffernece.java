...
public class PairDiffernece {
    ...
    static  List<int[]> question(int[]arr , int tar){
        ...
        for(int i = 0; i<n; i++){
            ...
            for(int j = i; j<n; j++){
                int pair = arr[j];
                int myTar = curr-pair;
                int myTarAbs = Math.abs(curr-pair);

                if(myTarAbs==tar){
                    if(myTar>0){
                        ans.add(new int[] {curr,pair});
                    }
                    else ans.add(new int[] {pair,curr});
                }           
            }
        }
    ...
    static List<int[]> pair (int[]arr , int tar){
       ...
       public static void main(String[] args) {
        int[] arr = { 4, 8, 6, 2 }; // [(6, 4), (8, 6), (4, 2)] - pair difference is 2 (target)
        int tar = 2;
        ...
        List<int[]> pairs = question(arr, tar);
